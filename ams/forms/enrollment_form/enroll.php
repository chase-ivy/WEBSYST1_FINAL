...
require_once __DIR__ . '/../../config/config.php';
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST'){
        ...
        $state1->execute([
            ...
            $_POST['Current_Zip_Code'] ?? ''
        ]);

        if (isset($_POST['same_address']) && $_POST['same_address'] === 'Yes') {

            $permanent_house   = $_POST['Current_House_No'] ?? '';
            $permanent_street  = $_POST['Current_Street_Name'] ?? '';
            $permanent_barangay= $_POST['Current_Barangay'] ?? '';
            $permanent_city    = $_POST['Current_Municipality_City'] ?? '';
            $permanent_province= $_POST['Current_Province'] ?? '';
            $permanent_country  = $_POST['Current_Country'] ?? '';
            $permanent_zip     = $_POST['Current_Zip_Code'] ?? '';

        } else {

            $permanent_house   = $_POST['Permanent_House_No'] ?? '';
            $permanent_street  = $_POST['Permanent_Street_Name'] ?? '';
            $permanent_barangay= $_POST['Permanent_Barangay'] ?? '';
            $permanent_city    = $_POST['Permanent_Municipality_City'] ?? '';
        ...
        $state3 = $pdo->prepare('INSERT INTO parents (last_name, first_name, middle_name, contact_number, parent_type) VALUES (?,?,?,?,?)');
        $state3->execute([$last, $first, $middle, $contact, $type]);
